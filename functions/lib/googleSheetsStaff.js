...
exports.FALLBACK_STAFF_DIRECTORY = exports.STAFF_DETAILS_TAB_NAME = exports.SPREADSHEET_ID = void 0;
exports.verifyStaffCredentials = verifyStaffCredentials;
const crypto_1 = __importDefault(require("crypto"));
exports.SPREADSHEET_ID = '19GO22yFFHR7fLbC8v4R8xifkI6f2fgdQbfQlvxfMHC0';
exports.STAFF_DETAILS_TAB_NAME = 'Staff Details';
exports.FALLBACK_STAFF_DIRECTORY = [
...
async function getGoogleSheetsAccessToken(credentials) {
    try {
...
async function verifyStaffCredentials(mobileNumber, password) {
    ...
    if (serviceAccount) {
        try {
            ...
            if (accessToken) {
                const encodedTab = encodeURIComponent(exports.STAFF_DETAILS_TAB_NAME);
                const url = `https://sheets.googleapis.com/v4/spreadsheets/${exports.SPREADSHEET_ID}/values/'${encodedTab}'!A:F`;
                ...
                if (res.ok) {
                    ...
                    if (rows.length >= 2) {
                        ...
                        let colEplId = headerRow.findIndex((h) => h.includes('epl') || h === 'id');
                        let colName = headerRow.findIndex((h) => h.includes('name'));
                        let colNumber = headerRow.findIndex((h) => h.includes('number') || h.includes('mobile') || h.includes('phone'));
                        let colEmail = headerRow.findIndex((h) => h.includes('email'));
                        let colPassword = headerRow.findIndex((h) => h.includes('pass'));
                        let colRole = headerRow.findIndex((h) => h.includes('role'));
                        if (colEplId === -1)
                            colEplId = 0;
                        if (colName === -1)
                            colName = 1;
                        ...
                        if (matchedUserRow) {
                            ...
                            const normalizedRole = rawRole.includes('admin') || rawRole.includes('operation') ? 'Admin' : 'Assistant';
                            return {
                                success: true,
                                role: normalizedRole,
        ...
        try {
            ...
            const scriptRes = await fetch(appsScriptUrl, {
                ...
                body: JSON.stringify({
                    ...
                    sheetName: exports.STAFF_DETAILS_TAB_NAME
                }),
                redirect: 'follow',
    ...
    const matched = exports.FALLBACK_STAFF_DIRECTORY.find((s) => {
...
//# sourceMappingURL=googleSheetsStaff.js.map